...
class NcEngine:
    ...
    def get_valid_band_files(self, rad_only=True):
        ...
        PID = str(os.getpid())
        logging.info(f'{PID} | S3 IMG folder: {sentinel_images_path}')
        logging.info(f'{PID} | Total files in folder: {len(files)}')
        logging.info(f'{PID} | Total NetCDF files: {len(nc_files)}')
        logging.info(f'{PID} | Total S3 "Oa" bands: {len(nc_bands)}')

        if rad_only:
            logging.info(f'{PID} | Done.')
            return nc_bands
        else:
            logging.info(f'{PID} | Done.')
            return nc_files
